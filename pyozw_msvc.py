...
batch files. There is a somiliar tool included with SetupTools and it is also
...
That is where this module comes into play. It is designed for ease of use.
    ...
    # notice we import distutils after the environment has been set
    # this is done intentionally. there is an Environment Variable that
    # #environment that has been created already

    # Python can be a fickle beast when it comes down to dealing with the how
    # extensions are built. The docs state that you need to compile a new
    # extension with the same compiler version that was used to compile Python.
    # I have personally found that this is not always true

    # by default the module will you the compiler version that was
    # used to compile Python.
    # You have the ability to set a minimum compiler version
    # or if you want a very specific compiler you have the
    # ability to do that as well


    from distutils.core import setup, Extension

    # if there are any preprocessor macros that need to be declared this is
    ...
    # create the extension instance that we pass to the
    s etup program in distutils

    extension = Extension(
        name='EXTENSION NAME GOES HERE',
        sources=['SOURCE_FILES_GO_HERE'],
        # Additional include directories you will add below.
        # This is also the spot that needs to have the Python include
        include_dirs=['ADDITIONAL INCLUDE DIRECTORIES GO HERE'],
        define_macros=define_macros,
        libraries=['LIBRARIES TO BE USED GO HERE],
        extra_objects=['EXTRA COMPILER_ARGUMENTS GO HERE],
        extra_compile_args=[
        ...
        extra_link_args=['EXTRA LINKER ARGUMENTS GO HERE],
        extra_objects=None,
        language='c++'
    )
    ...
    setup(
        ...
        version='1.1.1',
        name='EXAMPLE_SETUP',
        description='Example on how to use the msvc module',
        verbose=1,

        # this is where we pass the Extension instance
        ext_modules=[extension],

        # we set the command classes to be used
        cmdclass={'build_clib': BuildCLib, 'build_ext': build_ext}
        ,

        # we then set the options for each of the command classes
        options=dict(
            build_ext=dict(
