...
cdef extern from "Notification.h" namespace "OpenZWave::Notification":
    ...
    cdef enum UserAlertNotification:
        ...
        Alert_ConfigOutOfDate = 1               # A config file is out of date, use GetNodeId to determine which node(s) are effected.
        Alert_MFSOutOfDate = 2                  # A manufacturer_specific.xml file is out of date.
        Alert_ConfigFileDownloadFailed = 3      # A config file failed to download.
        Alert_DNSError = 4                      # An error occurred performing a DNS Lookup.
        Alert_NodeReloadRequired = 5            # A new config file has been discovered for this node, a node reload is required to have the new configuration take affect.
        Alert_UnsupportedController = 6         # The controller is not running a firmware library that is supported by OpenZWave.
        Alert_ApplicationStatus_Retry = 7       # The Application Status Command Class returned the message "Retry Later".
        Alert_ApplicationStatus_Queued = 8      # The command has been queued for later execution.
        Alert_ApplicationStatus_Rejected = 9    # The command was rejected.
