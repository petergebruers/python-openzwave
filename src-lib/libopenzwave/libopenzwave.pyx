...
from notification cimport Notification, NotificationType, NotificationCode, UserAlertNotification
...
PyUserAlerts = [
    EnumWithDoc('None').setDoc("No alert currently present."),
    EnumWithDoc('ConfigOutOfDate').setDoc("A config file is out of date, use GetNodeId to determine which node(s) are effected."),
    EnumWithDoc('MFSOutOfDate').setDoc("A manufacturer_specific.xml file is out of date."),
    EnumWithDoc('ConfigFileDownloadFailed').setDoc("A config file failed to download."),
    EnumWithDoc('DNSError').setDoc("An error occurred performing a DNS Lookup."),
    EnumWithDoc('NodeReloadRequired').setDoc("A new config file has been discovered for this node, a node reload is required to have the new configuration take affect."),
    EnumWithDoc('UnsupportedController').setDoc("The controller is not running a firmware library that is supported by OpenZWave."),
    EnumWithDoc('ApplicationStatus_Retry').setDoc("The Application Status Command Class returned the message \"Retry Later\"."),
    EnumWithDoc('ApplicationStatus_Queued').setDoc("The command has been queued for later execution."),
    EnumWithDoc('ApplicationStatus_Rejected').setDoc("The command was rejected."),
    ]
...
cdef void notif_callback(const_notification _notification, void* _context) with gil:
    ...
    if notification.GetType() == Type_Group:
        ...
        try:
            ...
            raise
    elif notification.GetType() == Type_UserAlerts:
        try:
            n['userAlert'] = PyUserAlerts[notification.GetUserAlertType()]
        except:
            logger.exception("notif_callback exception Type_UserAlerts")
            raise
