...
Use Flask-SocketIO : https://github.com/miguelgrinberg/Flask-SocketIO
/blob/master/example/app.py
Use socket-IO : https://github.com/abourget/gevent-socketio/blob/master
/examples/flask_chat/chat.py

Help : http://www.scratchinginfo.net
/useful-jquery-datatables-examples-tutorials-and-plugins/
...
from openzwave.option import ZWaveOption # NOQA

from .listener import start_listener, stop_listener # NOQA
...
def create_app(config_object='pyozwweb.config.DevelopmentConfig'):
    ...
    app.config['ZWAVE_DEBUG'] = zwave_debug
    ...
    if not hasattr(app, 'extensions'):
        app.extensions = {}
    if (
        'zwnetwork' not in app.extensions or
        app.extensions['zwnetwork'] is None
    ):
...
def start_zwnetwork(app):
    options = ZWaveOption(
        device=app.config['ZWAVE_DEVICE'],
