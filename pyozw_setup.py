    ...
    while True:
        try:
            ...
            if identifier == 'STDERR':
                sys.stderr.write('{0}\n'.format(line))
                log.error('{0}\n'.format(line))
            elif sys.platform.startswith("win") and progress_bar is not None:
                progress_bar.write(line)

    if sys.platform.startswith("win") and progress_bar is not None:
        progress_bar.close()
        progress_bar = None


def start_stream_watcher():
    Thread(
...
class Template(object):
    ...
    def cpp_context(self):
        ...
        exts['define_macros'] += [('PY_SSIZE_T_CLEAN', 1)]
        exts['sources'] = [
            "openzwave-embed/open-zwave-master/python-openzwave"
    ...
    def system_context(self, ctx, static=False):
        ...
        if sys.platform.startswith("win"):
            ...
            get_system_context(
                ctx,
                self.os_options,
                openzwave=os.path.abspath(self.openzwave),
                static=static
            )
            ...
            if static:
                ...
                ctx['extra_objects'] = [
                    "{0}/libopenzwave.a".format(self.openzwave)
                ]
                ctx['include_dirs'] += [
                    "{0}/cpp/build/linux".format(self.openzwave)
                ]
            else:
            ...
            if static:
                ctx['extra_objects'] = [
                    "{0}/libopenzwave.a".format(self.openzwave)]
                ctx['include_dirs'] += [
                    "{0}/cpp/build/mac".format(self.openzwave)]
            else:
                import pyozw_pkgconfig
            ...
            # Unknown system
            raise RuntimeError(
                "Can't detect plateform {0}".format(sys.platform)
            )
    ...
    def finalize_context(self, ctx):
        ...
        if self.flavor:
            ctx['define_macros'] += [
                ('PY_LIB_FLAVOR', self.flavor.replace('--flavor=', ''))
            ]
        else:
    ...
    def build(self):
        ...
        if (
            len(self.ctx['extra_objects']) == 1 and
            os.path.isfile(self.ctx['extra_objects'][0])
        ):
        ...
        if sys.platform.startswith("win"):
            ...
            for allowed_os in ("cygwin", "darwin", "linux"):
                ...
                # Unknown system
                raise RuntimeError(
                    "Can't detect platform {0}".format(sys.platform)
                )
    ...
    def install_so(self):
        ...
        if sys.platform.startswith("win"):
            proc = Popen(
                ['copy', 'OpenZWave.dll', '%SYSTEM32%\\'],
                stdout=PIPE,
                stderr=PIPE,
                cwd=self.os_options['vsproject_build']
            )
    ...
    def clean(self):
        ...
        try:
            ...
            return True
        log.info(
            "Clean openzwave in %s ... be patient ..." % (self.openzwave,)
        )
        ...
        if sys.platform.startswith("win"):
            ...
            # ~ proc.wait()
            # ~ proc = Popen(
            #   [ 'regsvr32', '-u', 'OpenZWave.dll' ],
            #   stdout=PIPE,
            #   stderr=PIPE,
            #   cwd='{0}'.format(os.path.abspath(self.openzwave))
            # )
            # ~ proc = Popen(
            #   [ 'del', '/F', '/Q', '/S', '%SYSTEM32%\OpenZWave.dll' ],
            #   stdout=PIPE,
            #   stderr=PIPE,
            #   cwd='{0}'.format(os.path.abspath(self.openzwave))
            # )
    ...
    def clean_all(self):
        ...
        if (
            ...
            (
                ...
                not os.path.isfile(os.path.join(dirn, 'device_classes.xml'))
            )
        ):
    ...
    def check_minimal_config(self):
        if sys.platform.startswith("win"):
            log.info(
                "Found MSBuild.exe : {0}".format(
                    self.os_options['msbuild_path']
                )
            )
            log.info("Found arch : {0}".format(self.os_options['arch']))
            log.info(
                "Found build configuration : {0}".format(
                    self.os_options['build_type']
                )
            )
            log.info(
                "Found Visual Studio project : {0}".format(
                    self.os_options['solution_path']
                )
            )
            log.info(
                "Found build path : {0}".format(self.os_options['build_path'])
            )
            ...
            if exe is not None:
                ...
                for lib in libs:
                    log.info(
                        "Found library "
                        "{0} : {1}".format(lib, pyozw_pkgconfig.exists(lib))
                    )
        ...
        try:
            ...
            for pyreq in self.build_requires():
                if pyreq not in packages:
                    ...
                    log.info(
                        "Minimal dependencies already "
                        "installed {0}".format(pyreq)
                    )
        except Exception:
    ...
    def get_openzwave(self,
        ...
        log.info(
            "fetching {0} "
            "into {1} "
            "for version {2}".format(url, dest_file, pyozw_version)
        )
    ...
    def clean_openzwave_so(self):
        for path in [
            ...
            '/usr/local/share/doc/openzwave'
        ]:
            try:
                log.info(
                    'Try to remove {0}'.format('/usr/local/etc/openzwave'))
...
class EmbedSharedTemplate(EmbedTemplate):
    ...
    def get_context(self):
        ...
        for ssubstitute in ['/', '/value_classes/', '/platform/']:
            incl = extra.replace('//', ssubstitute)
            if incl not in ctx['extra_compile_args']:
                ctx['extra_compile_args'] += [incl]
...
class build_openzwave(setuptools.Command):
    ...
    def finalize_options(self):
        if self.openzwave_dir is None:
            if (
                getattr(self, 'develop', False) or
                not getattr(self, 'install', False)
            ):
...
class develop(_develop):
    ...
    def run(self):
        # In case of --uninstall, it will build
        # openzwave to remove it ... stupid.
